...
# Make population data match morbidity 
ca_pop_strata <- ca_pop %>%
  mutate(
    ...
    # Standardize text for joins
    sex = toupper(sex)
  ) %>%
  # Re-code race labels to the numeric codes 
  mutate(
...
# Standardize combined morbidity data

combined_std <- combined %>%
  mutate(
    sex  = toupper(sex),
    race = as.numeric(race_eth)
  )


# Join morbidity & population -> create rate per 100,000 
...
head(
  joined_data_ca[, c("week_start", "county", "age_group", "sex", "race",
